...
import ast, os, json, warnings, io, base64, struct
warnings.filterwarnings("ignore")


def _chart(fig) -> dict:
    """Serialize Plotly figure to plain-dict, converting any bdata binary arrays
    to regular Python lists so Plotly.js receives standard JSON arrays."""
    def _decode(obj):
        if isinstance(obj, dict):
            if "bdata" in obj and "dtype" in obj:
                raw = base64.b64decode(obj["bdata"])
                dtype = obj["dtype"]
                fmt_map = {"f8": ("d", 8), "f4": ("f", 4),
                           "i4": ("i", 4), "u4": ("I", 4),
                           "i2": ("h", 2), "u2": ("H", 2)}
                if dtype in fmt_map:
                    fmt, size = fmt_map[dtype]
                    n = len(raw) // size
        ...
        return obj
    return _decode(json.loads(fig.to_json()))

_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
app = Flask(__name__,
            template_folder=os.path.join(_root, "templates"),
            static_folder=os.path.join(_root, "static") if os.path.isdir(os.path.join(_root, "static")) else None)
...
FEATURES = [
    "log_price", "price_per_guest", "price_per_bedroom",
    "accommodates","bedrooms","beds","bathrooms",
    "minimum_nights","availability_365","total_amenities","amenity_premium_score",
    ...
    "review_scores_value","reviews_per_month","review_volume_score","instant_bookable",
    "host_is_superhost","is_entire_home","neighbourhood_rank",
] + AMENITY_KEYS
...
def load_city(city: str, csv_path: str = None) -> pd.DataFrame | None:
    if csv_path is None:
        rel = CITIES.get(city, CITIES["bristol"])
        base = os.path.dirname(os.path.abspath(__file__))
        # Try Code/dataset/ first, then ../dataset/ (project root)
...
def _process_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["price_per_guest"] = df["price"] / df["accommodates"].clip(lower=1).fillna(1)
    df["price_per_bedroom"] = df["price"] / df["bedrooms"].clip(lower=0.5).fillna(1)
    df["review_volume_score"] = df["reviews_per_month"].fillna(0) * np.log1p(df["number_of_reviews"].fillna(0)) if "number_of_reviews" in df.columns else 0
    # Amenity premium score (weighted: pool/hot_tub/gym get 3x weight, others 1x)
    premium_weights = {"amen_pool": 3, "amen_hot_tub": 3, "amen_gym": 2, "amen_sauna": 2, "amen_waterfront": 2, "amen_harbor_view": 2}
    df["amenity_premium_score"] = sum(df.get(k, 0) * premium_weights.get(k, 1) for k in AMENITY_KEYS if k in df.columns)
...
def train_model(df: pd.DataFrame) -> dict:
    ...
    # KMeans clustering (use df not model_df since 'price' not in FEATURES)
    km_feats = ["price","estimated_revenue_l365d","availability_365","accommodates"]
    km_df = df[km_feats].dropna()
    ...
    for p in price_range:
        r = median_row.copy()
        r["log_price"] = np.log1p(p)
        r["price_per_guest"] = p / max(float(median_row.get("accommodates", 4)), 1)
        r["price_per_bedroom"] = p / max(float(median_row.get("bedrooms", 2)), 0.5)
        rev = gb.predict([r])[0]
        if rev > best_rev:
...
def build_input_row(prop: dict, artefacts: dict) -> pd.Series:
    ...
    nightly_price = float(prop.get("nightly_price", 100))
    accommodates  = float(prop.get("accommodates", 4))
    bedrooms      = float(prop.get("bedrooms", 2))
    base["neighbourhood_rank"]        = neigh_rank.get(prop.get("neighbourhood",""), 0.5)
    base["log_price"]                 = np.log1p(nightly_price)
    base["price_per_guest"]           = nightly_price / max(accommodates, 1)
    base["price_per_bedroom"]         = nightly_price / max(bedrooms, 0.5)
    base["accommodates"]              = accommodates
    ...
    num_reviews = float(prop.get("number_of_reviews", 0))
    base["review_volume_score"]       = float(prop.get("reviews_per_month", 2.0)) * np.log1p(num_reviews)
    ...
    for k in AMENITY_KEYS:
        ...
        if k in base.index:
            base[k] = v
        total_am += v
        premium_score += v * premium_weights.get(k, 1)
    base["total_amenities"] = total_am
    base["amenity_premium_score"] = premium_score
    return base


def compute_pl(revenue: float, prop: dict, price_override: float = None) -> dict:
    nightly       = price_override if price_override else float(prop.get("nightly_price", 100))
    bookings      = revenue / max(nightly * float(prop.get("avg_stay_length_nights", 3)), 1)
...
def compute_opportunities(row: pd.Series, current_rev: float,
    ...
    LEVERS = {
        ...
        "Optimise Nightly Price":          {"log_price": np.log1p(opt), "price_per_guest": opt/max(float(row["accommodates"]),1), "price_per_bedroom": opt/max(float(row["bedrooms"]),0.5)},
        "Respond Within 1 Hour":           {"host_engagement": min(float(row["host_engagement"])+0.15,1)},
        "Open 300+ Days/Year":             {"availability_365": 300},
...
def analyse():
    ...
    full["log_price"]                 = np.log1p(opt_p)
    full["price_per_guest"]           = opt_p / max(float(row["accommodates"]), 1)
    full["price_per_bedroom"]         = opt_p / max(float(row["bedrooms"]), 0.5)
    ...
    pl_opt  = compute_pl(full_rev,    prop, price_override=a["opt_price"])
