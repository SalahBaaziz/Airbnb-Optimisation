...
# ── Airbnb API constants ──────────────────────────────────────────────────────
AIRBNB_API_KEY = "d306zoyjsyarp7ifhu67rjxn52tv0t20"
# SHA-256 hashes for Airbnb's persisted GraphQL queries (valid as of 2025)
BOOK_IT_SHA    = "5490d8a97f4c12fa736d7db9cb17e15a1642c3e1da97d5b5674b5e6389c1a81e"
CALENDAR_SHA   = "b23335819df0dc391a338d665e2ee2f5d3bff19181d05c0b39bc6c5aac403914"


def _encode_listing_id(listing_id: str) -> str:
    """Encode a listing ID to Airbnb's GraphQL global ID format."""
    return base64.b64encode(f"StayListing:{listing_id}".encode()).decode()
...
def _fetch_page(url: str, timeout: int = 20) -> str | None:
    """Fetch the listing page. Seeds cookies via search page hit first."""
    session = requests.Session()
    # Extract base domain for seeding — use search page for more realistic cookie flow
    parsed = urlparse(url)
    base_url = f"{parsed.scheme}://{parsed.netloc}"
    search_url = f"{base_url}/s/United-Kingdom/homes"
    try:
        seed_headers = _make_headers()
        session.get(search_url, headers=seed_headers, timeout=10)
    ...
    for attempt in range(3):
        try:
            req_headers = _make_headers(referer=base_url)
            resp = session.get(url, headers=req_headers, timeout=timeout)
            if resp.status_code == 200:
                return resp.text
            if resp.status_code in (403, 429, 503):
                # Retry with a different User-Agent after backoff
                time.sleep((attempt + 1) * 4)
                retry_headers = _make_headers(referer=search_url)
                resp2 = session.get(url, headers=retry_headers, timeout=timeout)
...
def _parse_overview_text(text: str, result: dict):
    ...
    if re.match(r"studio", text) and "bedrooms" not in result:
        result["bedrooms"] = 0
    # Match "6 beds", "6 bed" but NOT "2 bedrooms" (bedroom handled above)
    m = re.match(r"(\d+)\s+beds?(?!\s*room)", text)
...
def _extract_from_niobe(data: dict, result: dict):
    ...
    if "nightly_price" not in result:
        # Try pdpDisplayPrice (PDP = Property Detail Page)
        pdp = _deep_get(data, "pdpDisplayPrice")
    ...
    if "nightly_price" not in result:
        # Try listingPrice key
        lp = _deep_get(data, "listingPrice")
        if lp is not None:
            p = _parse_price_string(str(lp))
            if p:
                result["nightly_price"] = p
    ...
    if "nightly_price" not in result:
        ...
        price_patterns = [
            r'"pdpDisplayPrice"[^}]{0,200}"price"\s*:\s*"([^"]+)"',
            r'"listingPrice"\s*:\s*"?(\d+(?:\.\d+)?)"?',
            r'"amount"\s*:\s*"(\d+(?:\.\d+)?)"',
            r'"price"\s*:\s*(\d+(?:\.\d+)?)\s*[,}]',
...
def _parse_sections(data: dict, result: dict):
    """
    Navigate directly to niobeClientData's stayProductDetailPage sections
    and extract data by sectionId. More reliable than generic deep search.
    """
    try:
        sections = (data.get("presentation", {})
    ...
    # Map of categoryType → result field for review sub-scores
    REVIEW_CAT_MAP = {
    ...
    for sec in sections:
        ...
        status     = sec.get("sectionContentStatus", "")
        section    = sec.get("section", {})
        if not isinstance(section, dict):
            continue

        # ── TITLE_DEFAULT → listing name ──────────────────────────────────
        if section_id == "TITLE_DEFAULT" and status == "COMPLETE":
            title = section.get("title") or section.get("listingTitle")
            if title and isinstance(title, str) and len(title) > 3:
                result["listing_name"] = title[:120]

        # ── LOCATION_DEFAULT → location string ────────────────────────────
        elif section_id == "LOCATION_DEFAULT" and status == "COMPLETE":
            subtitle = section.get("subtitle")
            if subtitle and isinstance(subtitle, str):
                result["location_hint"] = subtitle[:100]

        # ── REVIEWS_DEFAULT → overall + sub-scores ────────────────────────
        elif section_id == "REVIEWS_DEFAULT" and status == "COMPLETE":
            # Overall rating
            for k in ("overallRating", "localizedOverallRating", "starRating"):
                v = section.get(k)
                if v is not None:
                    try:
                        s = float(v)
                        ...
                        pass
            # Review count
            for k in ("reviewCount", "totalReviews", "numberOfReviews"):
                v = section.get(k)
                if v is not None:
                    try:
                        result["number_of_reviews"] = int(float(str(v)))
                        ...
                        pass
            # Sub-scores: [{categoryType: "CLEANLINESS", localizedRating: "4.9"}, ...]
            for r in section.get("ratings", []):
                if not isinstance(r, dict):
                    continue
                cat   = r.get("categoryType", "").upper()
                field = REVIEW_CAT_MAP.get(cat)
                if not field:
                    continue
                raw_val = r.get("localizedRating") or r.get("rating")
                if raw_val is not None:
                    try:
                        s = float(str(raw_val).replace(",", "."))
                        if 0 < s <= 5:
                            result[field] = round(s, 2)
                        elif 5 < s <= 10:
                            result[field] = round(s / 2, 2)
        ...
        # ── MEET_YOUR_HOST → isSuperhost ──────────────────────────────────
        elif section_id == "MEET_YOUR_HOST" and status == "COMPLETE":
            card = section.get("cardData", {})
            if isinstance(card, dict):
                v = card.get("isSuperhost")
                if v is not None:
                    result["is_superhost"] = 1 if v in (True, "true", 1) else 0

        # ── AMENITIES_DEFAULT → full amenity list ─────────────────────────
        elif section_id == "AMENITIES_DEFAULT" and status == "COMPLETE":
            titles = result.setdefault("_amenity_titles", [])
            for group_key in ("seeAllAmenitiesGroups", "previewAmenitiesGroups"):
        ...
        # ── HIGHLIGHTS_DEFAULT → self check-in flag ───────────────────────
        elif section_id == "HIGHLIGHTS_DEFAULT" and status == "COMPLETE":
            for h in section.get("highlights", []):
                if isinstance(h, dict) and h.get("type") == "LISTING_SELF_CHECKIN":
                    result.setdefault("_amenity_titles", []).append("self check-in")

        # ── BOOK_IT_SIDEBAR → canInstantBook (if COMPLETE on this page) ───
        elif section_id == "BOOK_IT_SIDEBAR" and status == "COMPLETE":
            v = section.get("canInstantBook")
            if v is not None:
                result["instant_bookable"] = 1 if v in (True, "true", 1) else 0


def _fetch_booking_section(session: requests.Session, listing_id: str,
                           base_url: str) -> bool | None:
    """
    Call Airbnb's secondary StaysPdpSections API to get canInstantBook.
    Returns True/False, or None on failure.
    """
    url = f"{base_url}/api/v3/StaysPdpSections/{BOOK_IT_SHA}"
    encoded_id = _encode_listing_id(listing_id)
    payload = {
    ...
    headers = {
        ...
        "Referer": f"{base_url}/rooms/{listing_id}",
        "User-Agent": random.choice(USER_AGENTS),
    }
    try:
        resp = session.post(url, json=payload, headers=headers,
                            params={"operationName": "StaysPdpSections",
                                    "locale": "en-GB", "currency": "GBP"},
                            timeout=10)
        if resp.status_code != 200:
            return None
        data = resp.json()
        # Navigate: data.data.presentation.stayProductDetailPage.sections.sections
        sections = (data.get("data", {})
                        ...
                        .get("sections", []))
        for sec in sections:
            if isinstance(sec, dict) and sec.get("sectionId") == "BOOK_IT_SIDEBAR":
                section = sec.get("section", {})
...
def _fetch_calendar_min_nights(session: requests.Session, listing_id: str,
                                base_url: str) -> int | None:
    """
    Call Airbnb's PdpAvailabilityCalendar API to get constantMinNights.
    Returns an int, or None on failure.
    """
    url = f"{base_url}/api/v3/PdpAvailabilityCalendar/{CALENDAR_SHA}"
    import datetime
    ...
    try:
        ...
        if resp.status_code != 200:
            return None
        data = resp.json()
        # Navigate: data.data.merlin.pdpAvailabilityCalendar.metadata.constantMinNights
        cal = (data.get("data", {})
                   .get("merlin", {})
                   .get("pdpAvailabilityCalendar", {}))
        meta = cal.get("metadata", {})
        if isinstance(meta, dict):
            v = meta.get("constantMinNights")
            if v is not None:
                return int(v)
        # Fallback: deep search
        v = _deep_get(data, "constantMinNights")
        if v is not None:
            return int(v)
        return None
    except Exception:
...
def scrape_listing(url: str, verbose: bool = True) -> dict:
    ...
    # Build a session so secondary API calls share cookies
    session = requests.Session()
    ...
    if source in ("niobe", "next_data", "inline") and data:
        # Section-based extraction first (most accurate)
        _parse_sections(data, result)
        # Generic deep-search extraction fills any remaining gaps
        _extract_from_niobe(data, result)

    # HTML fallback for anything still missing
    _html_fallback(html, result)

    # Secondary API calls for fields not available in page HTML
    if listing_id:
        # canInstantBook — only available via secondary API call
        if "instant_bookable" not in result:
            if verbose:
                print("  Fetching booking section (canInstantBook)...")
            can_instant = _fetch_booking_section(session, listing_id, base_url)
            if can_instant is not None:
                result["instant_bookable"] = 1 if can_instant else 0

        # minimum_nights — from availability calendar API
        if "minimum_nights" not in result or result.get("minimum_nights", 2) == 2:
            if verbose:
                print("  Fetching calendar (minimum_nights)...")
            min_nights = _fetch_calendar_min_nights(session, listing_id, base_url)
            if min_nights is not None:
                result["minimum_nights"] = min_nights
